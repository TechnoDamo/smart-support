...
`Base.metadata.create_all(...)` больше не используется. В docker-стеке миграции применяет отдельный контейнер, в тестах и локальных CLI-командах используется тот же Alembic-путь.
