...
```
        ┌────────────────┐
        │  Пользователь  │  (Telegram, web-widget)
        └──────┬─────────┘
               │ входящее сообщение
               ▼
 ┌────────────────────────────────┐
 │  backend (FastAPI)             │
 │  ├── Chats / Tickets           │
 │  ├── AI-оркестратор (full_ai)  │──► LLM (cloud / local)
 │  ├── Подсказки (ai_assist)     │──► Embeddings (cloud / local)
 │  ├── RAG (hybrid dense+sparse) │──► Qdrant
 │  ├── Outbox + Scheduler        │──► Postgres
 │  └── Object storage            │──► MinIO / filesystem
 └──────────┬─────────────────────┘
            │ REST
            ▼
 ┌────────────────────────────────┐
